...
export class CreateCommentsTable1731870320134 implements MigrationInterface {
    name = 'CreateCommentsTable1731870320134'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`article\` ADD CONSTRAINT \`FK_f4a9230d3fbea33f27dfe9007f6\` FOREIGN KEY (\`author\`) REFERENCES \`user\`(\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
        await queryRunner.query(`ALTER TABLE \`comment\` ADD CONSTRAINT \`FK_03a590c26b0910b0bb49682b1e3\` FOREIGN KEY (\`article_id\`) REFERENCES \`article\`(\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
        ...
        await queryRunner.query(`ALTER TABLE \`follow\` ADD CONSTRAINT \`FK_01f1145077ed46c13260797a42f\` FOREIGN KEY (\`following_user_id\`) REFERENCES \`user\`(\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
        await queryRunner.query(`ALTER TABLE \`follow\` ADD CONSTRAINT \`FK_015375c9fd9f40e3d5dd89a953e\` FOREIGN KEY (\`followed_user_id\`) REFERENCES \`user\`(\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
        await queryRunner.query(`ALTER TABLE \`like\` ADD CONSTRAINT \`FK_b2ca4afd6cb49fa0f22f83bce59\` FOREIGN KEY (\`article_id\`) REFERENCES \`article\`(\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
        await queryRunner.query(`ALTER TABLE \`like\` ADD CONSTRAINT \`FK_4356ac2f9519c7404a2869f1691\` FOREIGN KEY (\`user_id\`) REFERENCES \`user\`(\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
    }
